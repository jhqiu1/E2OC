...
[![English](https://img.shields.io/badge/English-README-ff69b4.svg)](README.md)
...
1. :bust_in_silhouette: **专家设计** &mdash; 成本高昂，领域特定，难以泛化
2. :wrench: **单算子 AHD** (EoH, FunSearch, ReEvo 等) &mdash; 独立优化每个算子，忽略耦合效应
3. :rocket: **E2OC (我们提出的方法)** &mdash; 首个通过 MDP 分析和 MCTS 搜索协同演化相互依赖算子的框架

<p align="center"><img src="docs/figures/figure_Com_pipeline_01.png" width="60%" alt="Pipeline Comparison"></p>

### :building_construction: E2OC 框架

![E2OC Framework](docs/figures/figure_framwork_01.png)

E2OC 包含四个核心组件：

:fire: **热启动初始化。** 使用算法生成器 $\mathcal{G}$ 对每个算子进行独立演化，构建结构化的**设计思想空间**，从精英算子中提取语义级的改进建议。这为每个算子预先构建了一组高质量的设计思想集合，将搜索空间从无界约束为可处理的。

:brain: **多领域设计思想语言空间。** 算子设计思想对应于语义级表示，每个思想定义了一个决策范式或改进方向。内部关系刻画了同一算子不同思想的相对优劣和继承关系，外部关系表示不同算子间的跨域依赖（互补、冲突或互斥）。MDP 用于分析这些耦合关系。

:mag: **渐进式 MCTS 搜索。** 探索跨算子设计思想的**组合**，识别有前景的设计策略。设计空间被建模为一棵树，节点状态代表不同领域的设计思想。基于 UCB 的节点选择平衡探索与利用。有界空间反而通过集中评估预算提升了性能。

:repeat: **算子轮换演化。** 在选定的设计策略下，每个算子在真实多算子系统的**上下文中**被演化和评估。轮换过程中通过逐一替换算子并评估其对整体性能的影响来逐步更新。**算法生成器可插拔**，基于 [LLM4AD](https://github.com/Optima-CityU/LLM4AD) 平台构建，支持多种方法，包括 **EoH、FunSearch、ReEvo 和 MCTS-AHD**。
...
框架将执行四组件协同演化流程：
1. :fire: **热启动**: 通过独立演化初始化算子种群
2. :brain: **语言空间**: 构建多领域设计思想空间
3. :mag: **渐进式 MCTS 搜索**: 探索设计策略组合
4. :repeat: **算子轮换演化**: 在上下文中协同演化算子
...
- :email: 通过邮箱联系我们: junhaoqiu2-c@my.cityu.edu.hk
- :people_holding_hands: 加入我们的团队 (即将推出)
